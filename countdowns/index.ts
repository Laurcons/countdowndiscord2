...
export function run() {
  ...
  function checkForTime() {
    ...
    // check last run
    const lastRunStr: string = (() => {
      try {
        return db.getData("/lastRun");
      } catch {
        return null;
      }
    })();
    ...
    let doWePerform = (() => {
      if (lastRun === null) {
        ...
        if (now.hour == 10 && now.minute == 30 && minutes * 60 > 65) {
          console.log("[SCHED] Performing, since it is 10:30");
          return true;
        }
        // has it been more than 1 day and 10 minutes since last run?
        if (minutes >= 10 + 60 * 24) {
          console.log(
            "[SCHED] Performing, since it's been more than 1 day and 10 mins"
          );
          return true;
        }
      }
